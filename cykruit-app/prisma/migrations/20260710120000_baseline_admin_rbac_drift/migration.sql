-- Baseline migration: captures schema changes already applied directly to this
-- dev DB via `prisma db push` during the Admin-identity/RBAC migration (Phase 8,
-- see docs/admin-ui/IMPLEMENTATION-PLAN-admin-ui.md) that never got a proper
-- migration file. This file is applied via `prisma migrate resolve --applied`
-- (marked as already-run, SQL below NOT executed against this DB — the objects
-- already exist here) so that migration history matches reality for any future
-- fresh install / CI run via `prisma migrate deploy`.
