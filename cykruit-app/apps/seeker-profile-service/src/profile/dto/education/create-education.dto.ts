...
import { IsString, IsNotEmpty, IsOptional, IsUUID, MinLength, MaxLength, Matches, ValidateIf } from 'class-validator';
...
export class CreateEducationDto {
    ...
    @IsUUID()
    @IsNotEmpty({ message: 'Provide either instituteId (existing) or instituteName (custom college)' })
    instituteId?: string;

    @ValidateIf((o) => !o.instituteId)
    @IsString()
    @IsNotEmpty({ message: 'Provide either instituteName (custom college) or instituteId (existing)' })
    @MinLength(2)
    @MaxLength(200)
