// apps/user-settings-service/src/settings/session/session-validator.service.ts
//
// CHANGES FROM PREVIOUS VERSION:
//   - generateDeviceFingerprint + compareFingerprints imported from @cykruit/auth-core
//   - req?: Request param now accepted (wired in from auth.guard.ts via interface)
//   - Fingerprint mismatch check added — LOW confidence = block + revoke session
//   - Medium confidence = allow through (minor browser update)
//   - Raw crypto import kept for token hashing + rotation (no dep on shared utils here)
...
export class SessionValidatorService implements ISessionValidator {
    ...
    async validateSession(
        ...
        ipAddress?: string,
        userAgent?: string,
        req?: Request, // ✅ wired in from AuthGuard → fingerprint check runs here too
    ): Promise<ISessionValidationResult> {

        // Hash the incoming raw token before DB lookup
        // auth-service stores SHA256(rawToken) in DB, cookie contains rawToken
        ...
        if (storedFingerprint && req) {
            ...
            if (!comparison.match && comparison.confidence === 'low') {
                // Revoke session — completely different browser/device
                await this.prisma.session.update({
                    where: { id: session.id },
                    data: {
                        isActive: false,
        ...
        if (session.sessionType === SessionType.COOKIE) {
            ...
            if (timeUntilExpiry < SESSION_ROTATION_THRESHOLD_MS || timeUntilExpiry < halfLife) {
                ...
                // Store the HASHED new token, return the RAW one to client
                ...
                newToken = rawNewToken; // raw token → AuthGuard sets it as cookie
            }
        }
