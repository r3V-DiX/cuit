...
import {
    ...
    Req, Res, HttpCode, HttpStatus, UseGuards, BadRequestException,
} from '@nestjs/common';
...
import { MobileLoginDto, RefreshTokenDto, UpdatePushTokenDto } from '../dto/device-session.dto';
...
export class SessionsController {
    ...
    async mobileLogin(@Body() dto: MobileLoginDto, @Req() req: Request) {
        ...
        const loginResult = await this.authService.verifyOtp(
            dto.email, dto.otp, false, ip, ua,
        );
    ...
    async refreshToken(@Body() dto: RefreshTokenDto, @Req() req: Request) {
        if (!dto.refreshToken) {
            throw new BadRequestException({ code: ErrorCodes.INVALID_TOKEN, message: 'Refresh token is required' });
        }
        const ip = sanitizeIpAddress(req.ip ?? req.socket.remoteAddress);
    ...
    async mobileLogout(
        ...
        const session = await this.sessionMobileService.findSessionByRefreshHash(dto.refreshToken, user.id);
        if (session) {
    ...
    async listSessions(@CurrentUser() user: User, @Req() req: Request) {
        ...
        const sessions = await this.sessionService.listUserSessions(user.id, currentToken);
    ...
    async revokeAllOtherSessions(@CurrentUser() user: User, @Req() req: Request) {
        ...
        const count = await this.sessionService.revokeAllOtherSessions(user.id, currentToken, { ip, userAgent: ua });
    ...
    async updatePushToken(
        ...
        await this.sessionService.updatePushToken(sessionId, user.id, dto.pushToken);
