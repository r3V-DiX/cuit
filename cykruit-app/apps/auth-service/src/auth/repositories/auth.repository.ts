...
import { User, AccountStatus, UserRole, DeviceType, SessionType } from '@prisma/client';
...
export class AuthRepository {
    ...
    async findUserWithEmployer(userId: string) {
    ...
    async createUser(data: {
        ...
        lastName: string; phone?: string; role: UserRole;
    }): Promise<User> {
    ...
    async markEmailAsVerified(userId: string): Promise<User> {
        return this.prisma.user.update({
            where: { id: userId },
            data: { isEmailVerified: true, emailVerifiedAt: new Date(), status: AccountStatus.ACTIVE },
        });
    }
    ...
    async rotateSession(oldToken: string, newToken: string, newExpiresAt: Date): Promise<void> {
        await this.prisma.session.update({
            where: { token: oldToken },
            data: { token: newToken, expiresAt: newExpiresAt, lastActivity: new Date(), isActive: true },
        });
    }
    ...
    async createVerificationToken(userId: string, hashedToken: string, expiresAt: Date) {
    ...
    async findToken(token: string) {
        return this.prisma.token.findUnique({ where: { token }, include: { user: true } });
    }

    async findUserTokensByType(userId: string, type: 'EMAIL_VERIFICATION' | 'PASSWORD_RESET' | 'LOGIN_OTP' | 'EMPLOYER_INVITE' | 'COMPANY_JOIN_REQUEST') {
    ...
    async createToken(data: {
        ...
        type: 'EMAIL_VERIFICATION' | 'PASSWORD_RESET' | 'LOGIN_OTP' | 'EMPLOYER_INVITE' | 'COMPANY_JOIN_REQUEST'; expiresAt: Date;
    }) {
        return this.prisma.token.create({ data });
    }

    async markTokenAsUsed(tokenId: string) {
        return this.prisma.token.update({ where: { id: tokenId }, data: { usedAt: new Date() } });
    }
    ...
    // OTP-based registration: no password, no auto-created employer row.
    // SEEKER gets a JobSeekerProfile immediately. EMPLOYER stays bare until KYC.
    async createUserWithLoginOtp(userData: {
        email: string; firstName: string; lastName: string;
        phone?: string; role: UserRole; otpHash: string; otpExpiresAt: Date;
    }) {
        return this.prisma.$transaction(async (tx) => {
            ...
            const otpToken = await tx.token.create({
                data: {
                    userId: user.id,
                    token: userData.otpHash,
                    type: 'LOGIN_OTP',
                    expiresAt: userData.otpExpiresAt,
                },
            });
            ...
            // EMPLOYER: no profile row at signup — created when they fill in company details

            return { user, otpToken, profile };
        });
    }

    async createLoginOtp(userId: string, otpHash: string, expiresAt: Date) {
        // Invalidate all previous unused OTPs for this user
        await this.prisma.token.updateMany({
            where: { userId, type: 'LOGIN_OTP', usedAt: null },
            data: { usedAt: new Date() },
        });
        return this.prisma.token.create({
            data: { userId, token: otpHash, type: 'LOGIN_OTP', expiresAt },
        });
    }

    async findLoginOtp(otpHash: string) {
        return this.prisma.token.findFirst({
            where: { token: otpHash, type: 'LOGIN_OTP', usedAt: null },
            include: { user: true },
        });
    }

    async markOtpUsedAndActivate(tokenId: string, userId: string) {
        return this.prisma.$transaction(async (tx) => {
            await tx.token.update({ where: { id: tokenId }, data: { usedAt: new Date() } });
    ...
    async resetPasswordAndCleanup(userId: string, hashedPassword: string) {
        return this.prisma.$transaction(async (tx) => {
            ...
            await tx.session.updateMany({
                where: { userId, isActive: true },
                data: { isActive: false, revokedAt: new Date(), revokedBy: 'password_reset' },
            });
    ...
    async changeEmailWithVerificationToken(
        ...
        return this.prisma.$transaction(async (tx) => {
            const user = await tx.user.update({
                where: { id: userId },
                data: { email: newEmail, isEmailVerified: false, emailVerifiedAt: null },
            });
            await tx.token.deleteMany({ where: { userId, type: 'EMAIL_VERIFICATION' } });
