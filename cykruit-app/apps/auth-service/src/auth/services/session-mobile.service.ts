...
export class SessionMobileService {
    ...
    async createMobileSession(
        ...
        const accessToken = jwt.sign(
            ...
            this.jwtSecret,
            { expiresIn: ACCESS_TOKEN_EXPIRY_STR, issuer: 'cykruit-auth', audience: 'cykruit-app' },
        );
        ...
        this.auditService.log(AuditAction.MOBILE_LOGIN, 'SUCCESS', userId, {
            ip: ipAddress,
            userAgent: deviceInfo?.deviceName || 'mobile',
        }, {
            platform: deviceInfo?.platform,
    ...
    async refreshMobileToken(
        ...
        this.auditService.log(AuditAction.MOBILE_TOKEN_REFRESHED, 'SUCCESS', session.userId, {
    ...
    async revokeMobileSession(
        ...
        await this.prisma.session.update({
            where: { id: sessionId },
            data: { isActive: false, revokedAt: new Date(), revokedBy: 'mobile_logout' },
        });
    ...
    private async enforceMobileSessionLimit(userId: string): Promise<void> {
        const sessions = await this.prisma.session.findMany({
            where: { userId, deviceType: DeviceType.MOBILE, sessionType: SessionType.JWT, isActive: true },
