...
export class CleanupService {
    ...
    async cleanExpiredSessions(): Promise<void> {
        try {
            ...
            if (count > 0) {
                this.logger.log(`[CLEANUP] Revoked ${count} expired session(s)`, 'CleanupService');
            }
        } catch (error) {
            this.logger.error('[CLEANUP] Failed to clean expired sessions', error?.stack, 'CleanupService');
        }
    }
    ...
    async cleanExpiredTokens(): Promise<void> {
        try {
            ...
            if (count > 0) {
                this.logger.log(`[CLEANUP] Deleted ${count} expired token(s)`, 'CleanupService');
            }
        } catch (error) {
            this.logger.error('[CLEANUP] Failed to clean expired tokens', error?.stack, 'CleanupService');
        }
    }
    ...
    async cleanOldRevokedSessions(): Promise<void> {
        try {
            ...
            this.logger.error('[CLEANUP] Failed to purge old revoked sessions', error?.stack, 'CleanupService');
        }
    }
    ...
    async cleanPendingDeletionAccounts(): Promise<void> {
        try {
            ...
            for (const user of usersToDelete) {
                ...
                this.logger.log(`[CLEANUP] Hard deleted account: ${user.id}`, 'CleanupService');
            }
            ...
            this.logger.error('[CLEANUP] Failed to hard delete pending accounts', error?.stack, 'CleanupService');
        }
    }
}