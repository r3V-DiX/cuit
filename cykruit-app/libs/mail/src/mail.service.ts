...
export class MailService {
    ...
    async sendOtpEmail(email: string, otp: string, expiryMinutes: number): Promise<void> {
        try {
            const { error } = await this.resend.emails.send({
                from: `"Cykruit" <${this.fromEmail}>`,
                replyTo: 'support@cykruit.com',
                to: email,
                subject: `${otp} is your Cykruit login code`,
                html: `
                    <div style="font-family: Arial, sans-serif; max-width: 480px; margin: 0 auto;">
                        <h2 style="color: #1a1a2e;">Your login code</h2>
                        <p>Use this code to sign in to Cykruit. It expires in <strong>${expiryMinutes} minutes</strong>.</p>
                        <div style="font-size: 36px; font-weight: bold; letter-spacing: 8px; color: #4f46e5; padding: 20px 0;">
                            ${otp}
                        </div>
            ...
            if (error) {
                this.logger.error('Failed to send OTP email', error.message, 'MailService');
    ...
    async sendEmployerInviteEmail(email: string, inviterName: string, companyName: string, inviteUrl: string): Promise<void> {
        try {
            const { error } = await this.resend.emails.send({
                from: `"Cykruit" <${this.fromEmail}>`,
                replyTo: 'support@cykruit.com',
                to: email,
                subject: `${inviterName} invited you to join ${companyName} on Cykruit`,
                html: `
                    <div style="font-family: Arial, sans-serif; max-width: 480px; margin: 0 auto;">
                        <h2 style="color: #1a1a2e;">You're invited!</h2>
                        <p><strong>${inviterName}</strong> has invited you to join the <strong>${companyName}</strong> team on Cykruit.</p>
                        <a href="${inviteUrl}" style="display: inline-block; background: #4f46e5; color: white; padding: 12px 24px; border-radius: 6px; text-decoration: none; font-weight: bold; margin: 16px 0;">
                            Accept Invitation
                        </a>
                        <p style="color: #666; font-size: 14px;">This invitation expires in 7 days. If you did not expect this, you can safely ignore this email.</p>
                    </div>
                `,
            });

            if (error) {
                this.logger.error('Failed to send employer invite email', error.message, 'MailService');
            } else {
                this.logger.log(`Employer invite email sent to ${email}`, 'MailService');
            }
        } catch (err) {
