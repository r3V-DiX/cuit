...
export class BullQueueService extends QueueService {
    ...
    async addJob<T>(queueName: string, jobName: string, data: T, options?: QueueJobOptions): Promise<void> {
        try {
            await this.getQueue(queueName).add(jobName, data, options);
            this.logger.log(`Job "${jobName}" added to queue "${queueName}"`, 'BullQueueService');
        } catch (error) {
            this.logger.error(`Failed to add job "${jobName}" to "${queueName}"`, error, 'BullQueueService');
            throw error;
        }
    }
    ...
    async addBulkJobs<T>(
        ...
        try {
            ...
            this.logger.log(`${jobs.length} bulk jobs added to "${queueName}"`, 'BullQueueService');
        } catch (error) {
            this.logger.error(`Failed bulk add to "${queueName}"`, error, 'BullQueueService');
            throw error;
        }
    }
    ...
    async removeJob(queueName: string, jobId: string): Promise<void> {
        ...
        if (job) {
            await job.remove();
            this.logger.log(`Job ${jobId} removed from "${queueName}"`, 'BullQueueService');
        }
    }

    async cleanQueue(queueName: string, grace: number = 3_600_000): Promise<void> {
