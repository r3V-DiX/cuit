...
export abstract class QueueService implements IQueueService {
    abstract addJob<T>(queueName: string, jobName: string, data: T, options?: QueueJobOptions): Promise<void>;
    abstract addBulkJobs<T>(queueName: string, jobs: Array<{ name: string; data: T; options?: QueueJobOptions }>): Promise<void>;
    ...
    abstract getQueueStats(queueName: string): Promise<{ waiting: number; active: number; completed: number; failed: number }>;
}