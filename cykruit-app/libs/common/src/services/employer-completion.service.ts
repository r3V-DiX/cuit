...
export class EmployerCompletionService {
  ...
  async calculateCompletion(
    employerOrId: string | any,
  ): Promise<EmployerCompletionResult> {
    let employer: any;
    ...
    const missingSections = Object.entries(sections)
      .filter(
        ([section, isComplete]) =>
          !isComplete && coreSections.includes(section),
      )
  ...
  private checkBranding(e: any): boolean {
    return !!(e.companyLogo && e.companyBanner);
  }
  private checkAbout(e: any): boolean {
    return !!(e.about && e.about.length >= 50 && (e.mission || e.vision));
  }
  private checkContact(e: any): boolean {
    return !!e.contactEmail;
  }
  private checkSocial(e: any): boolean {
    return !!(e.linkedin || e.twitter);
  }
  private checkVerification(e: any): boolean {
    return e.isVerified === true;
  }
  private checkTagline(e: any): boolean {
    return !!(e.tagline && e.tagline.length >= 10);
  }
  private checkCulture(e: any): boolean {
    return !!(e.cultureDescription && e.cultureDescription.length >= 50);
  }
  private checkInstagram(e: any): boolean {
    return !!e.instagram;
  }
  private checkOfficeLocations(e: any): boolean {
    return !!(e.officeLocations && e.officeLocations.length > 0);
  }
  private checkBenefits(e: any): boolean {
    return !!(e.benefits && e.benefits.length >= 3);
  }
  private checkTeamMembers(e: any): boolean {
    return !!(e.teamMembers && e.teamMembers.length > 0);
  }
  private checkCompanyMedia(e: any): boolean {
