...
import { AIProvider, AIGenerateOptions, AIGenerateResponse } from './ai-provider.interface';
...
export class GeminiProvider extends AIProvider {
    ...
    constructor(private configService: ConfigService) {
        ...
        this.modelName = this.configService.get<string>('ai.gemini.model', 'gemini-2.5-flash');
    ...
    async generate(prompt: string, options?: AIGenerateOptions): Promise<AIGenerateResponse> {
        try {
            ...
            return { text, usage: { inputTokens: usage.promptTokenCount || 0, outputTokens: usage.candidatesTokenCount || 0 } };
        } catch (error) {
            this.logger.error('Gemini API error:', error.message);
            throw new BadRequestException(`Gemini error: ${error.response?.data?.error?.message || error.message}`);
        }
    }
    ...
    async generateJobDescription(params: {
        ...
        const res = await this.generate(AI_PROMPTS.JOB_DESCRIPTION({ workMode: 'REMOTE', ...params }), { maxTokens: 4096, temperature: 0.7 });
        return res.text;
    }
