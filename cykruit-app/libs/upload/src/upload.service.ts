...
import { UploadOptions, UploadResult, DeleteFileOptions, BucketType } from './types/upload.types';
...
export class UploadService {
    ...
    async uploadFile(file: Express.Multer.File, options: UploadOptions): Promise<UploadResult> {
        ...
        if (!bucketName) throw new BadRequestException(`Bucket config missing: ${options.bucket}`);
        ...
        try {
            await this.s3Client.send(
                new PutObjectCommand({ Bucket: bucketName, Key: key, Body: file.buffer, ContentType: file.mimetype }),
            );
    ...
    async deleteFile(options: DeleteFileOptions): Promise<void> {
        ...
        try {
            await this.s3Client.send(new DeleteObjectCommand({ Bucket: bucketName, Key: options.key }));
        } catch (error) {
            throw new BadRequestException(`Failed to delete: ${error.message}`);
        }
    }

    async getPresignedUrl(key: string, bucketType: BucketType, expiresIn: number = 3600): Promise<string> {
        const bucketName = this.buckets[bucketType];
        if (!bucketName) throw new BadRequestException(`Bucket config missing: ${bucketType}`);
    ...
    async convertToPresignedUrl(fileUrl: string | null | undefined, expiresIn: number = 3600): Promise<string | null> {
    ...
    async transformFileUrls<T>(data: T, expiresIn: number = 3600): Promise<T> {
        ...
        if (Array.isArray(data)) {
            return Promise.all(data.map((item) => this.transformFileUrls(item, expiresIn))) as any;
        }
    ...
    private validateFile(file: Express.Multer.File, options: UploadOptions): void {
        ...
        if (file.size > maxBytes) throw new BadRequestException(`File exceeds ${options.maxSizeInMB}MB`);
        if (!options.allowedMimeTypes?.length) throw new BadRequestException('allowedMimeTypes required');
        if (!options.allowedMimeTypes.includes(file.mimetype))
            throw new BadRequestException(`Invalid type. Allowed: ${options.allowedMimeTypes.join(', ')}`);
    }

    extractKeyFromUrlSafe(fileUrl: string): string | null {
        try { return new URL(fileUrl).pathname.substring(1); } catch { return null; }
    }

    extractKeyFromUrl(fileUrl: string): string {
        try { return new URL(fileUrl).pathname.substring(1); } catch { throw new BadRequestException('Invalid file URL'); }
    }

    private extractBucketFromUrlSafe(fileUrl: string): string | null {
        try { return new URL(fileUrl).hostname.split('.')[0]; } catch { return null; }
    }

    extractBucketFromUrl(fileUrl: string): string {
        try { return new URL(fileUrl).hostname.split('.')[0]; } catch { throw new BadRequestException('Invalid file URL'); }
    }
