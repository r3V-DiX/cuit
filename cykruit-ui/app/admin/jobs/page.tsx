...
interface JobItem {
  ...
  status: string;
  createdAt: string;
  employer: { id: string; companyName: string; isVerified: boolean };
}
  ...
  return (
    <div className="p-6 space-y-5">
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-xl font-semibold text-white flex items-center gap-2">
            <Briefcase className="w-5 h-5 text-violet-400" /> Job Approvals
          </h1>
          <p className="text-sm text-slate-500 mt-0.5">Review and approve employer job listings</p>
        </div>
        <button onClick={fetchJobs} className="flex items-center gap-2 px-3 py-2 rounded-lg bg-white/5 hover:bg-white/10 text-slate-400 hover:text-white text-sm transition-all">
          <RefreshCw className={`w-4 h-4 ${loading ? "animate-spin" : ""}`} /> Refresh
        </button>
      </div>

      <form onSubmit={(e) => { e.preventDefault(); setPage(1); fetchJobs(); }}
      ...
      <div className="text-xs text-slate-500 font-mono">
        {meta.total.toLocaleString()} jobs · page {meta.page} of {meta.totalPages}
      </div>
              ...
              {items.map((job) => (
                <tr key={job.id} className="hover:bg-white/[0.02] transition-colors">
                  <td className="px-4 py-3">
                    <p className="text-slate-200 font-medium text-xs">{job.jobTitle}</p>
                  </td>
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-1.5">
                      <span className="text-slate-400 text-xs">{job.employer.companyName}</span>
                      {job.employer.isVerified && (
                        <span title="KYC verified"><CheckCircle2 className="w-3 h-3 text-emerald-400" /></span>
                      )}
                    </div>
                  </td>
                  <td className="px-4 py-3">
                    <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-md border text-[11px] font-medium ${STATUS_COLOR[job.status] ?? "text-slate-400 bg-white/5 border-white/10"}`}>
                      {job.status === "APPROVED" && <CheckCircle2 className="w-3 h-3" />}
                      {job.status === "REJECTED" && <XCircle className="w-3 h-3" />}
                      {job.status === "PENDING" && <AlertTriangle className="w-3 h-3" />}
                      {job.status}
                    </span>
                  </td>
                  <td className="px-4 py-3">
                    <span className="text-slate-400 text-xs">{relTime(job.createdAt)}</span>
                  </td>
                  <td className="px-4 py-3">
