...
const PLANS = [
  {
    ...
    monthlyPrice: 3999,
    yearlyPrice: 3199,
    description: "Perfect for small teams just getting started with cybersecurity hiring.",
    color: "blue",
    features: [
  ...
  },
  {
    ...
    monthlyPrice: 9999,
    yearlyPrice: 7999,
    description: "Built for scaling teams that need smarter hiring tools.",
    color: "violet",
    features: [
...
const FAQS = [
  {
    ...
    a: "Yes. You can upgrade or downgrade at any time from your employer dashboard. Upgrades are prorated and take effect immediately. Downgrades apply at the next billing cycle.",
  },
  {
    q: "Is there a free trial?",
    a: "The Growth plan includes a 14-day free trial — no credit card required. Starter and Enterprise plans can be evaluated via a demo call with our team.",
  },
  {
  ...
  },
  {
    ...
    a: "You'll be notified when you reach 80% of your limit. To post more listings you'll need to close existing ones or upgrade your plan.",
  },
  {
    q: "Do you offer discounts for non-profits or startups?",
    a: "Yes — we offer a 30% discount for verified non-profit organisations and early-stage startups (under 2 years old, under $2M funding). Contact our sales team to apply.",
  },
  {
    q: "How is billing handled for annual plans?",
    a: "Annual plans are billed upfront for 12 months and save you ~20% compared to monthly billing. An invoice is sent to your billing email on the charge date.",
  },
];
...
function PlanCard({ plan, yearly }: { plan: typeof PLANS[0]; yearly: boolean }) {
  ...
  const accentClass =
    plan.color === "violet"
      ? { border: "border-violet-300", ring: "ring-violet-200/60", badge: "bg-violet-100 text-violet-700 border-violet-200", btn: "bg-violet-600 hover:bg-violet-700 shadow-violet-500/20", check: "text-violet-500", top: "via-violet-500/60" }
      : { border: "border-blue-200", ring: "ring-blue-200/60", badge: "bg-blue-100 text-blue-700 border-blue-200", btn: "bg-blue-600 hover:bg-blue-700 shadow-blue-500/20", check: "text-blue-500", top: "via-blue-500/50" };

  return (
    <div className={`relative flex flex-col rounded-2xl border bg-white overflow-hidden transition-all duration-200 hover:-translate-y-0.5 hover:shadow-xl ${isPopular ? `${accentClass.border} ring-2 ${accentClass.ring} shadow-lg` : "border-slate-200 shadow-md"}`}>
      <div className={`absolute top-0 left-0 right-0 h-0.5 bg-linear-to-r from-transparent ${accentClass.top} to-transparent`} />

      {plan.badge && (
        <div className="absolute top-4 right-4">
          <span className={`text-[10px] font-mono font-bold px-2.5 py-1 rounded-full border ${accentClass.badge}`}>
            {plan.badge}
          </span>
        </div>
      )}
      ...
      <div className="p-7 pb-5">
        <div className="flex items-center gap-2 mb-3">
          {plan.color === "violet" ? <Zap className="w-4 h-4 text-violet-500" /> : <Shield className="w-4 h-4 text-blue-500" />}
        ...
        <Link
          href={`/employer/subscription/checkout?plan=${plan.id}&billing=${yearly ? "yearly" : "monthly"}`}
          className={`w-full flex items-center justify-center gap-2 h-11 rounded-xl text-white text-sm font-semibold transition-all shadow-md ${accentClass.btn}`}
        >
        ...
        <ul className="space-y-2.5">
          {plan.features.map((f) => (
            <li key={f} className="flex items-start gap-2.5 text-sm text-slate-700">
              <Check className={`w-4 h-4 shrink-0 mt-0.5 ${accentClass.check}`} />
              {f}
            </li>
          ))}
        </ul>
