      ...
      try {
        ...
        if (appsRes.ok) {
          ...
          setApplicants(items.map((a: any) => ({
            ...
            jobId: a.jobId,
            location: a.jobSeeker?.jobSeekerProfile?.location?.city ?? "—",
            experience: a.jobSeeker?.jobSeekerProfile?.experiences?.[0] ? `${a.jobSeeker.jobSeekerProfile.experiences[0].title}` : "—",
            status: appStatusMap[a.status] ?? "New",
            applied: new Date(a.appliedAt ?? a.createdAt).toLocaleDateString(),
            skills: (a.jobSeeker?.jobSeekerProfile?.skills ?? []).map((s: any) => s.skill?.name ?? s),
