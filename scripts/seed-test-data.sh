...
# ── 4. Rivedix employer ────────────────────────────────────────────────────────
step "Creating Rivedix employer (anshul.khaire@rivedix.com)..."
RIVEDIX_USER_ID=$(run_sql "SELECT id FROM users WHERE email='anshul.khaire@rivedix.com' LIMIT 1;")
...
if [ -z "$RIVEDIX_USER_ID" ]; then
  RIVEDIX_USER_ID=$(run_sql "
    ...
    VALUES (gen_random_uuid(), 'anshul.khaire@rivedix.com', '$RIVEDIX_HASH', 'Yograj', 'Hukumdar', 'EMPLOYER', 'ACTIVE', true, now(), now(), now())
    RETURNING id;")
...
echo "================================================"
echo ""
echo "  RIVEDIX (anshul.khaire@rivedix.com)"
