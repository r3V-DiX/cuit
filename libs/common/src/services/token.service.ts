...
export class TokenService {
    ...
    async validateEmailVerificationToken(rawToken: string): Promise<string> {
        ...
        const record = await this.prisma.token.findFirst({
            where: { token: hashedToken, type: TokenType.EMAIL_VERIFICATION, usedAt: null },
        });

        if (!record) throw new BadRequestException('Invalid or expired verification token');
        if (new Date() > record.expiresAt) throw new BadRequestException('Verification token has expired');
    ...
    async validatePasswordResetToken(rawToken: string): Promise<string> {
        ...
        const record = await this.prisma.token.findFirst({
            where: { token: hashedToken, type: TokenType.PASSWORD_RESET, usedAt: null },
        });

        if (!record) throw new BadRequestException('Invalid or expired reset token');
        if (new Date() > record.expiresAt) throw new BadRequestException('Reset token has expired');
