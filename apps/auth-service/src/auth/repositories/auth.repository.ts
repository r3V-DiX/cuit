...
export class AuthRepository {
    ...
    async findUserTokensByType(userId: string, type: 'EMAIL_VERIFICATION' | 'PASSWORD_RESET') {
    ...
    async createToken(data: {
        ...
        type: 'EMAIL_VERIFICATION' | 'PASSWORD_RESET'; expiresAt: Date;
    }) {
    ...
    async createUserWithVerificationToken(userData: {
        ...
        phone?: string; role: UserRole; verificationToken: string; tokenExpiresAt: Date;
    }) {
        return this.prisma.$transaction(async (tx) => {
            const user = await tx.user.create({
                data: {
                    ...
                    status: AccountStatus.PENDING, isEmailVerified: false,
                },
            });

            const token = await tx.token.create({
                data: {
                    userId: user.id, token: userData.verificationToken,
                    type: 'EMAIL_VERIFICATION', expiresAt: userData.tokenExpiresAt,
                },
            });
            ...
            if (userData.role === UserRole.SEEKER) {
                profile = await tx.jobSeekerProfile.create({
                    data: {
                        ...
                        firstName: userData.firstName, lastName: userData.lastName,
                        availability: 'Open to offers', profileCompletion: 0,
                    },
                });
                ...
                profile = await tx.employer.create({
                    data: {
                        user: { connect: { id: user.id } },
                        companyName: '', slug, companyType: 'OTHERS',
                        industry: 'OTHER', companySize: 'SIZE_1_10',
                        location: '', isVerified: false, profileCompletion: 0,
                    },
                });
            }
