...
export class PasswordService {
    ...
    async forgotPassword(
        ...
        this.auditService.log(AuditAction.PASSWORD_RESET_REQUESTED, 'SUCCESS', user.id, reqCtx);

        try {
            const resetUrl = `${this.configService.get('APP_URL')}/reset-password?token=${rawToken}`;
            await this.mailService.sendPasswordResetEmail(user.email, resetUrl);
        } catch (error) {
            this.logger.error(`Failed to send password reset email to ${user.email}`, error.stack, 'PasswordService');
        }
    ...
    async verifyResetToken(rawToken: string): Promise<{ valid: boolean; message: string }> {
        const hashedToken = this.hashService.hashToken(rawToken);
    ...
    async resetPassword(
        ...
        this.auditService.log(AuditAction.PASSWORD_RESET_COMPLETED, 'SUCCESS', tokenRecord.userId, reqCtx, {
    ...
    async changePassword(
        ...
        const isCurrentValid = await this.hashService.comparePassword(currentPassword, user.password);
        if (!isCurrentValid) {
        ...
        await this.prisma.$transaction(async (tx) => {
            ...
            if (currentSessionToken) {
                const hashedCurrent = createHash('sha256').update(currentSessionToken).digest('hex');
                await tx.session.updateMany({
                    where: { userId, isActive: true, token: { not: hashedCurrent } },
                    data: { isActive: false, revokedAt: new Date(), revokedBy: 'password_change' },
                });
            } else {
                await tx.session.updateMany({
        ...
        this.auditService.log(AuditAction.PASSWORD_CHANGED, 'SUCCESS', userId, reqCtx, {
