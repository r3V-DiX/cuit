...
export class GoogleOAuthService {
    ...
    async getAuthorizationUrl(role: UserRole, redirectUrl?: string): Promise<{ url: string; state: string }> {
        try {
            const state = await this.oauthBaseService.createState(OAuthProvider.GOOGLE, role, redirectUrl);
            ...
            this.logger.error('Failed to generate Google OAuth URL', error.stack, 'GoogleOAuthService');
            throw new InternalServerErrorException({ code: 'OAUTH_INIT_FAILED', message: 'Failed to initialize Google authentication.' });
        }
    }

    async handleCallback(code: string, state: string, ipAddress?: string, userAgent?: string) {
        const stateData = await this.oauthBaseService.validateAndConsumeState(OAuthProvider.GOOGLE, state);
    ...
    private async exchangeCodeForTokens(code: string): Promise<OAuthTokenResponse> {
        try {
            ...
            if (!tokens.id_token) {
                throw new UnauthorizedException({ code: 'GOOGLE_TOKEN_INVALID', message: 'Invalid response from Google.' });
            }
            return tokens as OAuthTokenResponse;
        } catch (error) {
            this.logger.error('Failed to exchange Google code for tokens', error.stack, 'GoogleOAuthService');
            if (error?.response?.code === 'GOOGLE_TOKEN_INVALID') throw error;
            throw new UnauthorizedException({ code: 'OAUTH_TOKEN_EXCHANGE_FAILED', message: 'Failed to authenticate with Google.' });
        }
    }

    private async verifyIdToken(idToken: string): Promise<OAuthUserData> {
        try {
            const ticket = await this.oauth2Client.verifyIdToken({ idToken, audience: this.clientId });
            const payload = ticket.getPayload();

            if (!payload || !payload.email) {
                throw new UnauthorizedException({ code: 'GOOGLE_ID_TOKEN_INVALID', message: 'Invalid Google ID token.' });
            }
            ...
            this.logger.error('Failed to verify Google ID token', error.stack, 'GoogleOAuthService');
            throw new UnauthorizedException({ code: 'GOOGLE_ID_TOKEN_INVALID', message: 'Failed to verify Google ID token.' });
        }
    }
    ...
    private validateConfig(): void {
        ...
        this.logger.log('Google OAuth configured successfully', 'GoogleOAuthService');
    }
}