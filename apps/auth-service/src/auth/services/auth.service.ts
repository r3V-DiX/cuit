...
export class AuthService {
    ...
    async register(dto: RegisterDto) {
        ...
        const { user } = await this.authRepository.createUserWithVerificationToken({
            ...
            verificationToken: hashedToken, tokenExpiresAt,
        });
        ...
        try {
            ...
            this.logger.error(`Failed to send verification email to ${user.email}`, error.stack, 'AuthService');
            return {
                warning: {
                    code: 'EMAIL_SEND_FAILED',
                    message: 'Account created but verification email could not be sent. Please request a new one.',
                },
            };
        }
    }
    ...
    async login(dto: LoginDto, ipAddress: string, userAgent: string, req?: Request) { // ✅ req added
        const reqCtx = { ip: ipAddress, userAgent };
        const user = await this.authRepository.findUserByEmail(dto.email);

        if (!user) {
            this.auditService.log(AuditAction.LOGIN_FAILURE, 'FAILURE', undefined, reqCtx, {
        ...
        if (user.lockedUntil && new Date() < user.lockedUntil) {
            ...
            this.auditService.log(AuditAction.LOGIN_LOCKED, 'FAILURE', user.id, reqCtx, {
        ...
        if (!user.password || user.password === '') {
            const providers = await this.prisma.userOAuthProvider.findMany({ where: { userId: user.id } });
            const names = providers.map((p) => p.provider.charAt(0) + p.provider.slice(1).toLowerCase());
        ...
        const isPasswordValid = await this.hashService.comparePassword(dto.password, user.password);
        ...
        if (user.status === AccountStatus.INACTIVE) {
            ...
            this.logger.log(`Account reactivated on login: ${user.email}`, 'AuthService');
        }
        ...
        if (user.status === AccountStatus.PENDING_DELETION) {
            ...
            this.logger.log(`Account deletion cancelled on login: ${user.email}`, 'AuthService');

            await this.authRepository.updateLastLogin(user.id, ipAddress);

            const sessionToken = await this.sessionService.createSession(
                user.id, dto.rememberMe || false, userAgent, ipAddress, req, // ✅ req passed
            );
            ...
            return {
                ...
                message: 'Welcome back! Your scheduled account deletion has been cancelled.',
                warning: {
                    code: 'DELETION_CANCELLED',
                    message: 'Your account deletion has been cancelled since you logged in.',
                },
            };
        }
    ...
    async getCurrentUser(userId: string) {
        ...
        if (user.role === UserRole.EMPLOYER) {
            ...
            if (employerStatus.profileCompletion < 100) {
                return { ...userData, info: { code: 'PROFILE_INCOMPLETE', message: `Your profile is ${employerStatus.profileCompletion}% complete. Complete it to unlock all features.` } };
            }
            ...
            if (employerStatus.needsVerification) {
                return { ...userData, info: { code: 'VERIFICATION_REQUIRED', message: 'Complete your company verification to start posting jobs.' } };
            }
            return userData;
        }
