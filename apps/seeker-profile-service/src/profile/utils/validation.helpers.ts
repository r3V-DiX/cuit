...
export class ValidationHelpers {
  ...
  static validateCurrentDateLogic(
    current: boolean,
    ...
    if (current && normalizedEndDate)
      throw new BadRequestException(
        `Current ${entityType} cannot have an end date`,
      );
    if (!current && !normalizedEndDate)
      throw new BadRequestException(
        `End date is required for past ${entityType}s`,
      );
  }

  static validateOptionalYearMonthRange(
  ...
  static validateOptionalYearRange(
    startDate?: string | null,
    endDate?: string | null,
  ): void {
  ...
  static validateMutuallyExclusive(
    ...
    if (field1 && field2)
      throw new BadRequestException(
        `Cannot provide both ${field1Name} and ${field2Name}`,
      );
  }

  static validateAtLeastOne(
    field1: any,
    field2: any,
    field1Name: string,
    field2Name: string,
    ...
    if (!field1 && !field2)
      throw new BadRequestException(
        `Must provide either ${field1Name} or ${field2Name}`,
      );
  }
}
