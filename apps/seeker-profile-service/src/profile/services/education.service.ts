  ...
  async createEducation(userId: string, dto: CreateEducationDto) {
    ...
    if (!dto.instituteId && !dto.instituteName) {
      throw new BadRequestException(
        "Provide either instituteId (existing college) or instituteName (custom college)",
      );
    }
    if (dto.instituteId && dto.instituteName) {
      throw new BadRequestException(
        "Provide either instituteId or instituteName, not both",
      );
    }
