...
export class LocationPreferenceService {
    ...
    async add(userId: string, dto: AddLocationPreferenceDto) {
        ...
        const locationExists = await this.locationPrefRepo.locationExists(dto.locationId);
        if (!locationExists) {
    ...
    async remove(userId: string, preferenceId: string) {
        const preference = await this.locationPrefRepo.findOneById(preferenceId, userId);
        if (!preference) {
            throw new NotFoundException({
        ...
        if (remaining.length > 0) {
            const updates = remaining.map((p, index) => ({ id: p.id, priority: index }));
